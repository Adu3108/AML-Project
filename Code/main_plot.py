...
class Trainer:
    def __init__(self, train_loader, val_loader, model_type, num_samples, epochs, patience, delta, batch_size, embed_size):
        # Hyperparameters
        ...
        # Model
        self.model = Model(z_dim=self.embed_size).to(self.DEVICE)
    ...
    def early_stopping(self, fold):
        ...
        if self.best_score == None:
            ...
            torch.save(model_state_dict, f"./weights/VAE/VAE_{self.embed_size}_{fold}_weights.pt")
...
class Classifier:
    def __init__(self, model, train_loader, val_loader, test_loader, epochs, num_classes, embed_size, sampling_strategy, delta, patience):
    ...
    def early_stopping(self, fold):
        ...
        with torch.no_grad():
            for image, label in tqdm(self.val_loader):
                ...
                embedding = self.model.encode(image, self.sampling_strategy)
        ...
        if self.best_score == None:
            ...
            torch.save(classifier_state_dict, f"./weights/classifier/classifier_{self.embed_size}_{fold}_weights.pt")
        elif self.best_score-val_loss>self.delta:
            self.best_score = val_loss
            classifier_state_dict = self.classifier.state_dict()
            self.num_bad_epochs = 0
    ...
    def predict(self, fold):
        self.model.load_state_dict(torch.load(f"./weights/VAE/VAE_{self.embed_size}_{fold}_weights.pt"))

        classifier_network = FeedForwardClassifier(output_dim=self.num_classes, embed_size=self.embed_size).to(self.DEVICE)
        classifier_network.load_state_dict(torch.load(f"./weights/classifier/classifier_{self.embed_size}_{fold}_weights.pt"))
...
sampling_strategies = ["MAP"]
model_types = ["VAE", "MMD"]

train_datasets = [MNIST_training_dataset]
        ...
        for sampling_strategy in sampling_strategies:
            ...
            for embed_size in embed_sizes:
                ...
                for fold, (train_idx, val_idx) in enumerate(kf.split(training_dataset)):
                    ...
                    model.load_state_dict(torch.load(f"./weights/VAE/VAE_{embed_size}_{fold}_weights.pt"))
            ...
            print("Ploting Latent Space")
            embed_size = embed_sizes[int(np.argmax(accuracies))]
            # Load the trained VAE Network
            model = Model(z_dim=embed_size).to(DEVICE)
            model.load_state_dict(torch.load(f"./weights/VAE/VAE_{embed_size}_1_weights.pt"))
            ...
            with torch.no_grad():
                for i, (image, label) in tqdm(enumerate(test_loader)):
                    ...
                    z = model.encode(image, sampling_strategy)
            ...
            plt.savefig(f"./figures/latent_new_{dataset_name}_{model_type}_{sampling_strategy}.png", format="png")

            df = pd.DataFrame(list(zip(*[embed_sizes, accuracies, best_accuracies])), columns=["Embedding Dimension", "Mean Accuracy", "Best Accuracy"])
            df.to_csv(f"./results/{dataset_name}_new2_{model_type}_{sampling_strategy}.csv")
